...
from ..config import eps, sapbert_path, sentbert_path, bert_path
...
class Event:
    ...
    def get_entities(self):
        ...
        for args in self.arg_vals:
            for arg in args:
                if arg is not None:
                    if arg.startswith("Nested_Event-"):
                        pass
                    elif arg.startswith("Pad: "):
                        entities += arg.lstrip("Pad: ").split(", ")
                        entities.append(arg)
                    else:
                        entities.append(arg)
        return entities
            ...
            if key in self.full_args_map:
                ...
                for id, val in enumerate(self.full_args_map[key]):
                    if self.self == (key, id):
                        ...
                        e.append(f"\"{event.get_embedding_sentEnc(id2event, tp).lower()[:-1]}\"")
                    else:
                        e.append(val)
                replace = ", ".join(e)
            sent = sent.replace(f"<{key}>", replace)
            if len(event_type.split(" ")) > 1 and "Speculation" in event_type.split(" ")[1]:
                sent += f" The event is a possible conjecture without firm evidence."
            if len(event_type.split(" ")) > 1 and "Negation" in event_type.split(" ")[1]:
                sent += f" The relation described in the event is the opposite."
            # if self.self is not None:
            #     sent += f" The entity plays a role as {self.self[0]} in this event."
        return sent
...
class BiomedicalBaseDataConfig(BaseDataConfig):
    ...
    def init_embeddings(self):
        if self.emb_method == "concat":
            ...
            # self_emb = np.zeros(embs_stacked.shape[1])
