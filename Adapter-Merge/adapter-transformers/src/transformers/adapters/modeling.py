...
class Adapter(nn.Module):
    ...
    def __init__(
        ...
        # Add identity linear projection for residual merge and final unmerge
        self.residual_merge_l = nn.Linear(self.input_size, self.input_size, bias=False)
        nn.init.eye_(self.residual_merge_l.weight)
        self.unmerge_l = nn.Linear(self.input_size, self.input_size, bias=False)
        nn.init.eye_(self.unmerge_l.weight)
    ...
    def pre_forward(
        ...
        # freeze (un)merge identity layer
        for param in self.residual_merge_l.parameters():
            param.requires_grad = False
        for param in self.unmerge_l.parameters():
            param.requires_grad = False
    ...
    def forward(self, x, residual_input, output_gating=False):
        ...
        residual_input = self.residual_merge_l(residual_input)
