...
if __name__ == '__main__':
    ...
    data_x = torch.rand(128, 3, 224, 224)
    data_y = torch.zeros(128)
    ...
    graph2 = resnet20(deepcopy(model2)).graphify()

    merge = ModelMerge(graph1, graph2, device="cpu")
    merge.transform(model3, dataloader, transform_fn=match_tensors_zipit)

    # graph1.draw(nodes=range(20), save_path="figs/graph1.png")
    # graph1.draw(nodes=range(len(graph1.G)-20, len(graph1.G)), save_path="figs/graph2.png")

    print(model.eval()(data_x))
    print(model2.eval()(data_x))
